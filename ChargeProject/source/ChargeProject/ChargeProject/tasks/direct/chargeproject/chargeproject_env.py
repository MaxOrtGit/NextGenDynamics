...
class ChargeprojectEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        # Look counter for leaving/entering look state
        self.look_counters = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        ...
        self.can_see = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        ...
        self._episode_sums = {
            ...
            for key in [
                ...
                #"look_los_reward",
                #"look_feet_movement_penalty",

                "attack_approach_reward",
                "attack_catch_reward",
    ...
    def _setup_scene(self):
        ...
        # Height scanner for terrain mapping
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        caught_mask = self._update_player_logic()
        ...
        to_attack = (self.robot_state == RobotState.PATROL) & self.can_see
        self.robot_state[to_attack] = RobotState.ATTACK

        attack_mask = (self.robot_state == RobotState.ATTACK)
        if torch.any(attack_mask):
            self.nav_targets[attack_mask] = player_pos[attack_mask]
    ...
    def _can_see_player(self, player_pos_w: torch.Tensor) -> torch.Tensor:
        ...
        return torch.norm(player_pos_w - self._robot.data.root_pos_w, dim=1) < self.cfg.robot_view_distance
    ...
    def _get_observations(self) -> dict:
        ...
        one_hot_state = torch.nn.functional.one_hot(self.robot_state, num_classes=5).float()
        ...
        obs = torch.cat(
            [
                ...
                for tensor in (
                    ...
                    self._actions,
                    far_staleness,
                    self.nav_targets - self._robot.data.root_pos_w,
                    self.can_see.unsqueeze(1).float(),
                    one_hot_state,
                )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        patrol_forward_vel_mapped = torch.exp(-lin_vel_error / 0.25)

        # --- Attack Specific Calculations ---
        attack_mask = (self.robot_state == RobotState.ATTACK).float()
        
        # 1. Approach Reward (Velocity projected towards player)
        robot_pos = self._robot.data.root_pos_w
        player_pos = self._player.data.root_pos_w
        
        target_vec = player_pos - robot_pos
        target_dir = target_vec / (torch.norm(target_vec, dim=1, keepdim=True) + 1e-6)
        
        # Dot product of velocity and direction to player
        # We use robot linear velocity in World Frame for this calculation
        lin_vel_w = self._robot.data.root_lin_vel_w
        approach_vel = torch.sum(lin_vel_w * target_dir, dim=1)
        
        # Only reward approach if we are in Attack mode
        approach_reward = attack_mask * approach_vel

        # 2. Catch Reward (Binary bonus for being very close)
        # We calculated 'caught_mask' in pre_physics, but rewards are calc'd post_physics.
        # We can re-calculate dist or cache the caught_mask. 
        # Re-calculating is safer to ensure it aligns with the current step's state.
        dist_to_player = torch.norm(robot_pos[:, :2] - player_pos[:, :2], dim=1)
        catch_reward = (dist_to_player < 0.65).float() * attack_mask

        rewards = {
            # Patrol specific rewards
            "patrol_forward_vel": patrol_forward_vel_mapped * self.cfg.patrol_velocity_matching * self.step_dt,
            ...
            "attack_approach_reward": approach_reward * self.cfg.attack_approach_reward_scale * self.step_dt,
            "attack_catch_reward": catch_reward * self.cfg.attack_catch_reward_scale * self.step_dt,
        ...
        if self.cfg.log:
            ...
            # log percentage of robots in each state
            for state in RobotState:
                state_mask = (self.robot_state == state)
                percent_in_state = torch.sum(state_mask).item() / self.num_envs
                self._log_data(f"State_Percentage/{state.name}", percent_in_state)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        net_contact_forces = self._contact_sensor.data.net_forces_w_history
        died = torch.any(torch.max(torch.norm(net_contact_forces[:, :, self.base_contact_ids], dim=-1), dim=1)[0] > 1.0, dim=1)

        # Kill if base is below certain height
        base_height = self._robot.data.root_pos_w[:, 2]
        died = died | (base_height < -5.0)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # 5. Reset Game Logic / AI State
        self.robot_state[env_ids] = RobotState.PATROL
        self.state_timers[env_ids] = 0.0
        self._player_movement_angle[env_ids] = 0.0
    ...
    def _update_player_logic(self):
        ...
        # move slightly downwards to simulate slight falling
        velocity[:, 2] = -0.5
        ...
        # 2. Check for "Death" (Catch)
        # We use a simple distance check. If robot is within 0.6m of player, it's a catch.
        robot_pos = self._robot.data.root_pos_w
        player_pos = self._player.data.root_pos_w
        
        # Calculate 2D distance (ignore height differences slightly)
        dist = torch.norm(robot_pos[:, :2] - player_pos[:, :2], dim=1)
        
        # Threshold: Robot Radius (~0.4) + Player Radius (0.25)
        caught_mask = dist < 0.65 

        # 3. Respawn Caught Players
        if torch.any(caught_mask):
            caught_env_ids = caught_mask.nonzero(as_tuple=False).flatten()
            self._respawn_players(caught_env_ids)

        return caught_mask
    
    def _respawn_players(self, env_ids):
        if len(env_ids) == 0:
            return
        ...
        # 2. Random Radius (25m to 35m)
        # r = min + (rand * (max - min))
        r = 25.0 + (torch.rand(len(env_ids), device=self.device) * 10.0)
        
        # 3. Calculate new X/Y positions relative to origin
        new_pos = torch.zeros((len(env_ids), 3), device=self.device)
        new_pos[:, 0] = origins[:, 0] + r * torch.cos(alpha)
        new_pos[:, 1] = origins[:, 1] + r * torch.sin(alpha)
        new_pos[:, 2] = 3.25 # Standard player height
        
        # 4. Reset rotation (Upright)
        new_rot = torch.zeros((len(env_ids), 4), device=self.device)
        new_rot[:, 3] = 1.0 # w component (Identity quaternion)

        # 5. Write to simulation
        self._player.write_root_pose_to_sim(torch.cat([new_pos, new_rot], dim=-1), env_ids)
        
        # 6. Reset velocity to zero
        zeros_vel = torch.zeros((len(env_ids), 6), device=self.device)
        self._player.write_root_velocity_to_sim(zeros_vel, env_ids)
